...
// importing resources
import userImage from "../resources/images/logo.png"
...
const AppBar = styled(MuiAppBar, {
  ...
  paddingLeft: '1%',
  boxShadow: 'none',
  background: 'linear-gradient(135deg,#18d061, #00f7ff, #00f7ff)',
  ...
  return (
    <Box className="HomeContainer">
      ...
      <AppBar className="AppBar" position="fixed" open={drawerOpen}>
        ...
        <IconButton
          ...
          <Box className="Profile" sx={{mr: 2}}>
              <Avatar alt="sashank" src={userImage} sx={{cursor: 'pointer', border: '3px solid rgb(243, 185, 13)',width: 50, height: 50}} onClick={handleClick} />
              <Menu
                id="basic-menu"
                anchorEl={anchorEl}
                open={openAvatarMenu}
      ...
      <DrawerHeader className={`${drawerOpen ? "show sideFloatClass": "hide"}`} >
          <Avatar alt={userName} src={userImage} sx={{cursor: 'pointer', margin: 'auto', width: 100, height: 100 }} />
          <h1  className="lobster "style={{marginLeft: 'auto', marginRight: 'auto',color : 'rgb(243, 185, 13)'}} >{userName}</h1>
          <IconButton onClick={handleDrawerClose} style={{ color : 'rgb(243, 185, 13)'}}>
                    ...
                    <Tooltip title={data.title} placement='right' >
                      <ListItemIcon
                        sx={{
                          ...
                          color: index === activeSideMenu ? 'white' : 'rgb(243, 185, 13)',
                        }}
                      >
                        {data.icon}
                      </ListItemIcon>
                    </Tooltip>
                    <ListItemText primary={data.title} sx={{ opacity: drawerOpen ? 1 : 0 ,color: index === activeSideMenu ? 'black' : 'white',}} />
                  </ListItemButton>
                </ListItem>  
              </Link>
            </div>
