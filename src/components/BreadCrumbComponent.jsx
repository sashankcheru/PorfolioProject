...
export default function BreadCrumbComponent({data}) {
    return (
        // <Box sx={{p:3}}>
        <Box>
            <Breadcrumbs aria-label="breadcrumb" separator={<NavigateNextRounded sx={{color:'white'}}/>}>
            ...
            {/* <Breadcrumbs aria-label="breadcrumb"> */}
                {
                    data.map((data, index) => (
                        <div className="breadCrumbItem" key={index} >
                            <Link 
                                fontSize={12}
                                sx={{cursor: 'pointer', display: 'flex', alignItems: 'center', pr: 1, pl: 1, textAlign:'çenter'}} 
